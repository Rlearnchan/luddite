...
VALID_APPEND_MODES = {DEFAULT_APPEND_MODE, STAGING_APPEND_MODE}
...
def validate_ops_safety(
    *,
    ...
    if mode == STAGING_APPEND_MODE and sheet != JIBI_STAGING_SHEET:
        raise ValueError(
            "staging_append may only target the exact `jibi 후보` sheet."
        )
