...
JIBI_BOARD_SUPPORT_SEARCH_FLAG ?= --no-support-search
JIBI_BOARD_SUPPORT_SEARCH_CATEGORIES ?= news,webkr
JIBI_BOARD_SUPPORT_SEARCH_MAX_CALLS ?= 60
JIBI_BOARD_SUPPORT_SEARCH_RESULTS_PER_QUERY ?= 5
JIBI_BOARD_SUPPORT_LINKS_PER_ROW ?= 1
JIBI_REPLAY_DATES ?= 2026-05-27,2026-05-31,2026-06-01
JIBI_USE_QUALITY_FLOOR ?= 0
JIBI_ARTICLE_BODY_CACHE ?= data/jibi/article_cache/article_bodies.jsonl
JIBI_ARTICLE_BODY_MAX_ITEMS ?= 40
JIBI_ARTICLE_BODY_FETCH_FLAGS ?=
JIBI_LLM_JUDGE ?= 0
...
jibi-manual-update:
	JIBI_DATE=$(JIBI_DATE) JIBI_RSS_INBOX=$(JIBI_RSS_INBOX) \
		...
		JIBI_USE_QUALITY_FLOOR=$(JIBI_USE_QUALITY_FLOOR) \
...
jibi-review-board-refresh-with-syuka:
	...
	PYTHONPATH=$(PYTHONPATH) $(VENV_PYTHON) -m luddite refresh-jibi-review-board-with-syuka \
		...
		$(JIBI_BOARD_SUPPORT_SEARCH_FLAG) \
		--support-search-categories $(JIBI_BOARD_SUPPORT_SEARCH_CATEGORIES) \
		--support-search-max-calls $(JIBI_BOARD_SUPPORT_SEARCH_MAX_CALLS) \
		--support-search-results-per-query $(JIBI_BOARD_SUPPORT_SEARCH_RESULTS_PER_QUERY) \
		--support-links-per-row $(JIBI_BOARD_SUPPORT_LINKS_PER_ROW)
...
jibi-quality-replay:
	PYTHONPATH=$(PYTHONPATH) $(VENV_PYTHON) -m luddite replay-jibi-quality \
		--dates $(JIBI_REPLAY_DATES)
...
jibi-second-search-intake:
	PYTHONPATH=$(PYTHONPATH) $(VENV_PYTHON) -m luddite.agents.jibi.second_search_intake \
		--date $(JIBI_DATE)

jibi-hidden-support-search:
	PYTHONPATH=$(PYTHONPATH) $(VENV_PYTHON) -m luddite.agents.jibi.hidden_support_search \
		--date $(JIBI_DATE) \
		--provider $(JIBI_SEARCH_PROVIDER) \
		--categories $(JIBI_HIDDEN_SUPPORT_SEARCH_CATEGORIES) \
		--max-calls $(JIBI_HIDDEN_SUPPORT_SEARCH_MAX_CALLS) \
		--results-per-query $(JIBI_HIDDEN_SUPPORT_SEARCH_RESULTS_PER_QUERY) \
		--max-links-per-row $(JIBI_HIDDEN_SUPPORT_LINKS_PER_ROW)

jibi-hidden-support-intake:
	PYTHONPATH=$(PYTHONPATH) $(VENV_PYTHON) -m luddite.agents.jibi.hidden_support_intake \
		--date $(JIBI_DATE)
...
jibi-article-body-fetch:
	PYTHONPATH=$(PYTHONPATH) $(VENV_PYTHON) -m luddite fetch-jibi-article-bodies \
		--date $(JIBI_DATE) \
		--output-jsonl $(JIBI_ARTICLE_BODY_CACHE) \
		--max-items $(JIBI_ARTICLE_BODY_MAX_ITEMS) \
		$(JIBI_ARTICLE_BODY_FETCH_FLAGS)

jibi-evidence-pack:
	PYTHONPATH=$(PYTHONPATH) $(VENV_PYTHON) -m luddite build-jibi-evidence-pack \
		--date $(JIBI_DATE) \
		--article-cache $(JIBI_ARTICLE_BODY_CACHE)

jibi-llm-editorial-judge:
	JIBI_LLM_JUDGE=$(JIBI_LLM_JUDGE) \
		JIBI_LLM_JUDGE_MODEL=$(JIBI_LLM_JUDGE_MODEL) \
		JIBI_LLM_JUDGE_MAX_ITEMS=$(JIBI_LLM_JUDGE_MAX_ITEMS) \
