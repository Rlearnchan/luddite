...
리뷰보드 포맷은 [Jibi Review Board Format Contract](docs/product/jibi_review_board_format_contract.md)에 고정합니다.
...
- `Jibi` 단일 탭 상단 1-4행에는 Jibi 안내문을 둡니다.
- 표 헤더는 안내문 아래에 두고, 후보는 그 다음 행부터 시작합니다.
- 제목은 항상 한국어 질문형 또는 설명형 제목으로 씁니다.
- `설명`은 왜 골랐는지, 어떤 이야기로 자랄 수 있는지, 무엇을 더 봐야 하는지를 자연스러운 문단으로 씁니다.
- `과거 영상`에는 syuka-ops 유사 영상처럼 후보 판단의 보조 정보만 넣습니다.
- 실제 Google Sheet에서는 영상 제목 부분에 YouTube 링크를 겁니다.
- syuka-ops 유사 영상 제목, 날짜, 조회수, 좋아요 수는 `설명`에 섞지 않습니다.
- `서브 링크`는 묶인 후보나 근거 링크만 짧게 둡니다.
